...
- 여러 개의 일치하는 펀스크립트 변형이 있을 때 `Soft` 같은 라벨까지 구분해서 고를 수 있는 빠른 스크립트 변형 패널 추가
- Repeat 버튼이 다시 현재 미디어 반복 전용으로 동작하고, 다음 파일 자동 재생은 별도 `자동 다음 재생` 설정으로 분리
- 트랙 끝부분에서 재생 시간 표시 길이가 늦게 늘어나는 문제 수정
- 폴더 열기 시 스캔을 더 가볍게 하고 오래된 캐시를 무효화해 멈칫거림을 줄임
- 랜덤 폴백 스트로크 이름을 정리해 더 강한 프리셋과 기본 애태우기 패턴이 같은 이름으로 보이지 않도록 수정
